...
def find_matching_segment(pattern_api, pat_idx, p0, p1, tol_mm=8.0):
    """Return (segment_index, is_reversed) for the edge closest to p0->p1, or None."""
    try:
        pts = pattern_api.GetPatternPoint2DList(pat_idx)
    except Exception:
        return None
    n = len(pts)
    if n < 2:
    ...
    for i in range(n):
        a = pts[i]
        b = pts[(i + 1) % n]
        d_fwd = dist2d(a, p0) + dist2d(b, p1)
        d_rev = dist2d(a, p1) + dist2d(b, p0)
        d = min(d_fwd, d_rev)
        if d < best_d:
            best_d = d
            best = (i, d_rev < d_fwd)
...
def create_seams(pattern_api, seams_data, frag_to_pattern):
    """Create CLO sewing lines from manifest seams list."""
    if not seams_data:
        return 0
    created = 0
    for seam in seams_data:
        frag_ids = seam.get("fragmentIds", [])
        pts = seam.get("points", [])
        if len(frag_ids) < 2 or len(pts) < 2:
            continue
        fid_a, fid_b = str(frag_ids[0]), str(frag_ids[1])
        pat_a = frag_to_pattern.get(fid_a)
        pat_b = frag_to_pattern.get(fid_b)
        if pat_a is None or pat_b is None:
            log(f"  seam skip: pattern not found for frags {fid_a}/{fid_b}")
            continue
        p0 = (float(pts[0]["x"]), float(pts[0]["y"]))
        p1 = (float(pts[1]["x"]), float(pts[1]["y"]))
        seg_a = find_matching_segment(pattern_api, pat_a, p0, p1)
        seg_b = find_matching_segment(pattern_api, pat_b, p0, p1)
        if seg_a is None or seg_b is None:
            log(f"  seam skip: no matching segment for frags {fid_a}/{fid_b}")
            continue
        try:
            pattern_api.CreateSewingBetweenMultiplePatternLine(
                [pat_a, pat_b],
                [seg_a[0], seg_b[0]],
                [seg_a[1], seg_b[1]],
                [0.0, 0.0],
            ...
            log(f"  seam error frags {fid_a}/{fid_b}: {e}")
    return created
...
def run_import(zip_path, fabric_api, pattern_api):
    ...
    for i, entry in enumerate(entries):
        ...
        try:
            ...
            frag_to_pattern[str(frag_id)] = pat_idx
            created += 1
    ...
    seams_data = manifest.get("seams", [])
    if seams_data:
        log(f"Creating {len(seams_data)} seams...")
        seams_created = create_seams(pattern_api, seams_data, frag_to_pattern)
        log(f"Seams: {seams_created} / {len(seams_data)} created.")
    else:
        log("No seams in manifest (seamMode=none or no shared edges).")
